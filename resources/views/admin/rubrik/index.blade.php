...
@section('content')
    <section class="panel" style="margin-bottom:20px;">
        <header class="panel-heading" style="color: #ffffff;background-color: #074071;border-color: #fff000;border-image: none;border-style: solid solid none;border-width: 4px 0px 0;border-radius: 0;font-size: 14px;font-weight: 700;padding: 15px;">
            <i class="fa fa-tasks"></i>&nbsp;Manajemen rubrik Remunerasi Insentif Universitas Bengkulu
        </header>
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <form action="{{ route('admin.rubrik.post') }}" method="POST">
                {{ csrf_field() }} {{ method_field('POST') }}
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-info-new alert-block">
                            <i class="fa fa-info-circle"></i>&nbsp; Silahkan tambahkan rubrik baru jika diperlukan.
                        </div>
                    </div>
                    <div class="form-status_periode col-md-6">
                        <label>Nama Rubrik</label>
                        <input type="text" name="nama_rubrik" id="nama_rubrik" class="form-control @error('nama_rubrik') is-invalid @enderror">
                        @error('nama_rubrik')
                       ...
                       <tbody>
                           ...
                           @foreach ($rubriks as $rubrik)
                               <tr>
                                   ...
                                   <td>{{ $rubrik->periode_pembayaran }}</td>
                                   <td>
                                    @if($rubrik->status == "aktif")
                                ...
                                <td>
                                 @if ($rubrik->status == "aktif")
                                     <form action="{{ route('admin.rubrik.non_aktifkan_status', [$rubrik->id]) }}" method="POST">
                                         {{ csrf_field() }} {{ method_field('PATCH') }}
                                         <button type="submit" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-thumbs-down"></i></button>
                                     </form>
                                     @else
                                     <form action="{{ route('admin.rubrik.aktifkan_status', [$rubrik->id]) }}" method="POST">
                                         {{ csrf_field() }} {{ method_field('PATCH') }}
                                ...
                                <td>
                                    <form action="{{ route('admin.rubrik.delete') }}" method="POST">
